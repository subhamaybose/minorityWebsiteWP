...
//----------------------------------------------------------------------------------
if ( ! function_exists( ( 'im_minority_blog_customize_comments' ) ) ) {
	function im_minority_blog_customize_comments( $comment, $args, $depth ) { ?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			...
			<div class="comment-content">
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<span
						class="awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'indian-minority' ) ?></span>
					<br/>
				<?php endif; ?>
				<?php comment_text(); ?>
			</div>
			<div class="comment-footer">
				...
				<?php edit_comment_link(
					esc_html__( 'Edit', 'indian-minority' ),
...
//----------------------------------------------------------------------------------
if ( ! function_exists( 'im_mnority_blog_update_fields' ) ) {
	function im_mnority_blog_update_fields( $fields ) {
...
add_filter( 'comment_form_default_fields', 'im_mnority_blog_update_fields' );

//----------------------------------------------------------------------------------
// Update the comment textarea field
//----------------------------------------------------------------------------------
if ( ! function_exists( 'im_minority_blog_update_comment_field' ) ) {
	function im_minority_blog_update_comment_field( $comment_field ) {
...
add_filter( 'comment_form_field_comment', 'im_minority_blog_update_comment_field' );

//----------------------------------------------------------------------------------
// Remove comment notes with markup that displays after the comment form
//----------------------------------------------------------------------------------
if ( ! function_exists( 'im_minority_blog_remove_comments_notes_after' ) ) {
	function im_minority_blog_remove_comments_notes_after( $defaults ) {
		$defaults['comment_notes_after'] = '';
		return $defaults;
	}
}
add_action( 'comment_form_defaults', 'im_minority_blog_remove_comments_notes_after' );