...
if ( $author_display == 'no' ) {
	...
	printf( esc_html_x( 'Published on %s', 'This blog post was published on some date', 'indian-minority' ), esc_html( $date ) );
} elseif ( $date_display == 'no' ) {
	// translators: %s = the author who published the post
	printf( esc_html_x( 'Published by %s', 'This blog post was published by some author', 'indian-minority' ), esc_html( $author ) );
} else {
	// translators: %1$s = the date the post was published. %2$s = the author who published it
	printf( esc_html_x( 'Published on %1$s by %2$s', 'This blog post was published on some date by some author', 'indian-minority' ), esc_html( $date ), esc_html( $author ) );
}
echo '</div>';
